...
async function loadProfileOverview() {
    ...
    try {
        const userRes = await fetch(`http://localhost:5001/profile/${user.userId}`);
...
async function renderSimpleOrgChart() {
    ...
    try {
        const userRes = await fetch(`http://localhost:5001/profile/${user.userId}`, { signal });
        if (!userRes.ok) throw new Error("Failed to load profile");
        const self = await userRes.json();
        const selfCompanyId = self.companyId ?? self.company_id;

        const [registryRes, usersRes] = await Promise.all([
            fetch(`http://localhost:5001/employee_registry`, { signal }),
            fetch(`http://localhost:5001/users`, { signal })
        ]);
